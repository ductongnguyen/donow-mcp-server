...
function writeMarker(repoPath, data) {
...
function logCurrentProject() {
    if (CURRENT_PROJECT) {
        console.error(`DoNow MCP: bound to project "${CURRENT_PROJECT.name}" via ${CURRENT_PROJECT.source} (projectId=${CURRENT_PROJECT.projectId || "-"}, externalId=${CURRENT_PROJECT.externalId}, repo=${CURRENT_PROJECT.repoPath})`);
    }
    else {
